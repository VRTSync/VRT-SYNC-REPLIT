AdminRouter.register('communities', async function(container, params) {
  ...
  let allCommunities = [];
  let allSummaries = {};
  let orgMap = {}; // id → name, fetched once for org-name chips
  ...
  function renderGrid(filterQ) {
    ...
    grid.innerHTML = list.map(c => {
      ...
      const orgName = c.organizationId ? orgMap[c.organizationId] : null;
      return `
        <div class="community-card" data-id="${c.id}">
          <div class="community-card-header">
            <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">
              <h3>${esc(c.name)}</h3>
              ${orgName ? `<span class="community-org-chip">${esc(orgName)}</span>` : ''}
            </div>
  ...
  async function loadCommunities() {
    try {
      // Fetch communities and org names in parallel (single extra request for org map)
      const [list, orgs] = await Promise.all([
        apiFetch('/api/communities'),
        apiFetch('/api/admin/organizations').catch(() => []),
      ]);
      allCommunities = list;
      orgMap = {};
