...
// Resolve asset directories — works both in dev (CWD = artifact dir) and in
// prod (assets are copied next to the compiled bundle).
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

function resolveDir(relative: string): string {
  // In production the bundle lives in dist/ alongside copies of templates/ and public/.
  const fromBundle = path.resolve(__dirname, relative);
  if (fs.existsSync(fromBundle)) return fromBundle;
  // In dev, process.cwd() is the artifact root.
  return path.resolve(process.cwd(), relative);
}

const PUBLIC_ROOT = resolveDir("public");
const TEMPLATES_ROOT = resolveDir("templates");
...
// ── Session (reads sessions set by the API server) ──────────────────────────
...
// ── Start ─────────────────────────────────────────────────────────────────────
app.listen(PORT, () => {
  logger.info({ port: PORT }, "Web portal server listening");
});
