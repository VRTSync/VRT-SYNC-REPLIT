...
export async function getPortfolioBranchDetail(orgId: string, communityId: string) {
  ...
  const [layersResult, inventoryResult, recentServicesResult, openWorkOrdersResult, groupIdsResult, servicesTotalResult, photoProofResult] = await Promise.all([
    ...
    pool.query<{ id: string; name: string; type: string; sub_layer_key: string | null; asset_count: string; color: string | null; stroke_color: string | null; stroke_weight: string | null; fill_opacity: string | null; has_geometry: boolean }>(`
      SELECT
        ...
        (ml.geojson_data IS NOT NULL AND jsonb_array_length(COALESCE((ml.geojson_data::jsonb -> 'features'), '[]'::jsonb)) > 0) AS has_geometry
      FROM map_layers ml
  ...
  return {
    ...
    layers: layersResult.rows.map(r => ({
      ...
      name: r.name,
      type: r.type,
      subLayerKey: r.sub_layer_key ?? null,
      assetCount: Number(r.asset_count),
      color: r.color ?? null,
      ...
      hasGeometry: r.has_geometry ?? false,
    })),
