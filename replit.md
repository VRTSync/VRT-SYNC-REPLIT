...
- **Visit status semantics**: `scheduled` = expected/not yet performed; `completed` = performed (`completed_at` set); `skipped` = intentionally skipped, reason in `skipReason`; `missed` = expected date passed with no completion and no reason.
- **Commercial tier (organizations/branches)**: A commercial branch *is* a community — branches are rows in the existing `communities` table with an added `organization_id` FK and optional `code`/`address`/`city` fields. Work orders *are* tasks — approval fields (`estimate_cents`, `approved_at`, `approved_by`) extend the existing `tasks` table. New tables only for genuinely new concepts: `organizations` (the portfolio owner) and `branch_groups`/`branch_group_members` (logical groupings of branches). Deleting an org sets `communities.organization_id` and `users.organization_id` to NULL (set null, not cascade). A partial unique index enforces uniqueness of `(organization_id, code)` only when both are NOT NULL.
...
- **Shared code duplication**: Metro still can't resolve files outside the mobile artifact's project root, so any *new* shared pure-logic file would need a manual copy into `artifacts/vrtsync-mobile/shared/` — but no such copies exist today; `assetFieldTemplates.ts` is served over `/api/asset-types` instead (see Gotchas for details).
