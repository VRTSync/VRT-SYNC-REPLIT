...
- **Startup migrations**: `index.ts` runs raw `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` statements on boot rather than a migration tool, preserving the original app's approach for additive schema changes.
