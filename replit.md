...
- **Global Search** — GET /api/search?q=&communityId=&types= searches across assets (label, featureRef, key properties like serialNumber/species/brand) and tasks (title, description, address) with relevance ranking. Permission-aware: contractors see only their communities/assigned tasks, admins see all. Frontend SearchModal component with debounced input, grouped results by type (Assets/Tasks), and navigation to detail views. Accessible from Home and Tasks tab headers via search icon.
